...
class InternetStateSensor(SensorEntity):
    ...
    async def _check_single_url(self, url: str, timeout: float = 5.0) -> bool:
        ...
        try:
            ...
            try:
                ...
                _LOGGER.error("Failed to connect to URL %s. Error: %s", url, err)
    ...
    async def _is_any_url_reachable(self, urls: list[str], timeout: float = 5.0) -> bool:
        ...
        for result in results:
            if isinstance(result, bool) and result:
                ...
                _LOGGER.error("Exception during URL checking: %s", result)
    ...
    async def async_update(self) -> None:
        ...
        _LOGGER.debug("--- Checking all URL lists in parallel ---")
        ...
        _LOGGER.debug(f"Check results: Global={global_check}, Russia={russia_check}, Whitelist={whitelist_check}")
